 ...
 * @returns pass: true if UPDATE query is successful, false if no user with that login or error returned from the DB
 */
async function modUser(login, user) {
  ...
  await db.query(`UPDATE hauth_user SET (${cols}) = (${dols}) WHERE login='${login}' RETURNING *`, vals, function(err, result) {
    if(err){
      console.log(err);
    }else{
      console.info(`UPDATED ${result.rowCount} rows....`);
      (result.rowCount) > 0 ? pass=true : pass=false
      return pass
    }
  });
  return pass
 ...
 * @param {*} user 
 * @returns pass: true if DELETE query is successful, false if no user with that login or error returned from the DB
 */
async function delUser(user){
  ...
  await db.query(`DELETE FROM hauth_user WHERE login='${user.login}'`, function(err, result) {
    if(err){
      ...
      console.info(`DELETED ${result.rowCount} rows....`);
      (result.rowCount) > 0 ? pass=true : pass=false
      return pass
    }
  });
